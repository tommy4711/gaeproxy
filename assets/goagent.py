...
class Common(object):
    ...
    def select_gae_ip(self, verify):
        ...
        hostslist = [x.split('|') for x in schemeval.split('||')]
        ...
        if verify:
            ...
            gae_ip = random_choice(hostslist[0])
            gae_port = port
        self.GAE_SERVER = '%s:%s' % (gae_ip if '.' in gae_ip else '[%s]' % gae_ip, gae_port)
    ...
    def select_fetchserver(self, url):
        ...
        if not common.GAE_PROXY or common.GAE_PREFER == 'https':
            gae_fetchserver = '%s://%s%s' % (self.GAE_PREFER, self.GAE_SERVER, self.GAE_PATH)
        else:
            gae_fetchserver = '%s://%s%s' % (self.GAE_PREFER, gae_host, self.GAE_PATH)
        return gae_fetchserver, gae_host

    def show(self):
        '''show current config'''
        print '--------------------------------------------'
        print 'OpenSSL Mode : %s' % {True:'Enabled', False:'Disabled'}[self.OPENSSL]
        ...
        print 'GAE Host     : %s%s' % (self.GAE_SERVER, '' if self.GAE_VERIFY else ' (NOT Verify)')
        print '--------------------------------------------'
...
class RootCA(object):
    ...
    def getCertificate(self, host):
        ...
        if not os.path.isfile(keyFile) or not os.path.isfile(crtFile):
            ...
            self.writeFile('/sdcard/cert/serial', self.SERIAL)
        return (keyFile, crtFile)
...
ROOTCA = RootCA() if common.OPENSSL else DummyRootCA()
    ...
    def _fetch(self, url, method, headers, payload):
        ...
        for i in range(1, 3):
            try:
                ...
                logging.debug('GaeProxyHandler fetch %r from (%r, %r)', url, gae_host, gae_server)
                ...
                if selected:
                    self.log_message('_fetch errors(%r), common.select_gae_ip(1) again' % selected)
                    common.select_gae_ip(common.GAE_VERIFY)
                    common.show()
...
class ConnectProxyHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def _direct(self, host, port, hosts):
        ...
        try:
            ...
            hostslist = [hosts[i:i+RandomTCPConnection.CONNECT_COUNT] for i in xrange(0,len(hosts),RandomTCPConnection.CONNECT_COUNT)]
            for hosts in hostslist:
                conn = RandomTCPConnection(hosts, port)
                if conn.socket is not None:
                    break
