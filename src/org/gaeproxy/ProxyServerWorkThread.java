...
public class ProxyServerWorkThread implements Runnable {
	...
	public void run() {
		...
		try {
			...
			// headers done
			resp.append("Accept-Ranges: none");
			resp.append("\n");
			...
			byte[] unzipped;
			if (isText) {
				byte[] zipped = bout.toByteArray();
				unzipped = unzip(zipped);
			} else {
				unzipped = bout.toByteArray();
			}
			Log.d(TAG, "--------- unzip begin---------------");
			Log.d(TAG + " unzip ", unzipped.toString());
			Log.d(TAG, "--------- unzip end---------------");
			os.write(resp.toString().getBytes());
			os.write(unzipped);
			os.flush();
